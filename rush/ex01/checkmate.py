#added knight moves
#added pawn transformation

transformation_printed = False

def checkmate(board):

    global transformation_printed
    transformation_printed = False
    ...
    for i in range(row):
        for j in range(row):
            if board[i][j] == "R":  
                ...
                if knight_attack(board, i, j, king_row, king_col):
                    return True
            else:
                continue
    return False
...
def pawn_attack(board, pawn_row, pawn_col, king_row, king_col):
    if pawn_row - 1 == king_row and (pawn_col - 1 == king_col or pawn_col + 1 == king_col):
        return True
    if pawn_transformation(board, pawn_row, pawn_col, king_row, king_col):
        return True
    return False

def pawn_transformation(board, pawn_row, pawn_col, king_row, king_col):
    global transformation_printed
    
    if pawn_row == 0 and (len(board) == 8 and len(board[0]) == 8):
        if queen_attack(board, pawn_row, pawn_col, king_row, king_col):
            if not transformation_printed:
                print(f"Pawn({pawn_row},{pawn_col}) transformed to Queen")
                transformation_printed = True
            return True
        elif knight_attack(board, pawn_row, pawn_col, king_row, king_col):
            if not transformation_printed:
                print(f"Pawn({pawn_row},{pawn_col}) transformed to Knight")
                transformation_printed = True
            return True
        
    return False

def knight_attack(board, knight_row, knight_col, king_row, king_col):
    knight_moves = [
        (2, 1), (2, -1), (-2, 1), (-2, -1),
        (1, 2), (1, -2), (-1, 2), (-1, -2)
    ]
    ...
    for r, c in knight_moves:
        ...
        if n_row == king_row and n_col == king_col:
            return True
